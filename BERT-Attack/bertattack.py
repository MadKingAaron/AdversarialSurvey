...
def run_attack():
    ...
    parser.add_argument("--maskedLM", type=str, help="Masked LM model to use to generate the substitutions")
    ...
    maskedLMName = str(args.maskedLM)

    maskedLM = model_zoo.maskedLMs[maskedLMName]
    #seq_classifer = model_zoo.seqClassifiers[maskedLMName]

    print('start process:',maskedLMName)
    ...
    # tokenizer_mlm = BertTokenizer(vocab_file="./vocab/bert-base-uncased-vocab.txt", do_lower_case=True)
    # tokenizer_tgt = BertTokenizer(vocab_file="./vocab/bert-base-uncased-vocab.txt", do_lower_case=True)
    
    
    tokenizer_mlm = maskedLM[0].from_pretrained(maskedLM[2]) 
    #tokenizer_mlm = BertTokenizer.from_pretrained('bert-base-uncased')
    tokenizer_tgt = BertTokenizer.from_pretrained("textattack/bert-base-uncased-imdb")
    #tokenizer_tgt = seq_classifer[0].from_pretrained(seq_classifer[2]) 

    #config_atk = BertConfig.from_pretrained(mlm_path)
    #mlm_model = BertForMaskedLM.from_pretrained(mlm_path, config=config_atk)
    config_atk = BertConfig()
    mlm_model = maskedLM[1].from_pretrained(maskedLM[2]) 
    #mlm_model = BertForMaskedLM.from_pretrained("bert-base-uncased")
    mlm_model.to('cuda')

    config_tgt = BertConfig(num_labels=num_label)
    # tgt_model = BertForSequenceClassification(config=config_tgt)
    ...
    tgt_model = BertForSequenceClassification.from_pretrained("textattack/bert-base-uncased-imdb")
    # tgt_model = seq_classifer[1].from_pretrained(seq_classifer[2])
    tgt_model.to('cuda')
